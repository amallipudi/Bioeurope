...
it('Check weather Enable Single or multiple Document Upload feature in BIO Europe Logistics App. ', (done) =>{
        ...
        cy.get('#cphContent_cphContent_rdDealType > tbody > tr > :nth-child(2) > label').click().wait(1000); // deal type
   ...
   cy.get('select[name="ctl00$ctl00$cphContent$cphContent$ddlBrokerFeeUOM"]').select('Metric Tons',{force: true}).wait(1000)
    ...
    cy.get('#txtBxSupplierLoadLocation').type('india123456789')
    cy.get('#cphContent_cphContent_dvddlGPSupplier > .btn-group > .btn > .filter-option').click();// GP suplier
    cy.get('#cphContent_cphContent_dvddlGPSupplier > .btn-group > .open > .bs-searchbox > .form-control').type('ADM INTERNATIONAL SARL{enter}').wait(3000)
   ...
   cy.get('select[name="ctl00$ctl00$cphContent$cphContent$ddlProduct"]').and('contain',"[ Fame -10 ] - Fame -10").and('contain',"[ Fame -5 ] - Fame -5")//product dropdown
    ...
    cy.get('#txtContractStartDate').type('29-10-2023').click({force: true})
    cy.get('#txtContractEndDate').type('31-10-2023').click({force: true})
    ...
    cy.get('select[name="ctl00$ctl00$cphContent$cphContent$ddlDocumentType"]').select('Purchase Contract',{force: true}).wait(1000)
    cy.get('#tbxDocumentNotes').type('hello test ananda')
    cy.get('#fuUploadDocuments').should("be.visible")  
